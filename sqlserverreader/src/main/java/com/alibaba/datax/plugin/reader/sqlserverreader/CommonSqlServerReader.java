...
public class CommonSqlServerReader {
    ...
    public static class Task {
        ...
        public void startRead(Configuration readerSliceConfig,
            ...
            String maxSql = readerSliceConfig.getString("maxSql");
            ...
            try {
                ...
                if (StringUtils.isNotBlank(maxSql)) {
                    maxRs = DBUtil.query(conn, maxSql, fetchSize);
                    ResultSetMetaData maxData = maxRs.getMetaData();
                    while (maxRs.next()) {
                        switch (maxData.getColumnType(1)) {
                            case Types.CHAR:
                            case Types.NCHAR:
                            case Types.VARCHAR:
                            ...
                            case Types.REAL:
                            case Types.DOUBLE:
                                LOG.info("read max is: {}", maxRs.getString(1));
                                break;
                            case Types.TIME:
                                LOG.info("read max is: {}", maxRs.getTime(1).toString());
                                break;

                            // for mysql bug, see http://bugs.mysql.com/bug.php?id=35115
                            case Types.DATE:
                                LOG.info("read max is: {}", maxRs.getDate(1).toString());
                                break;

                            case Types.TIMESTAMP:
                                LOG.info("read max is: {}", maxRs.getTimestamp(1).toString());
                                break;
                            ...
                            case Types.LONGVARBINARY:
                                LOG.info("read max is: {}", maxRs.getBytes(1));
                                break;
                            case Types.BOOLEAN:
                            case Types.BIT:
                                LOG.info("read max is: {}", maxRs.getBoolean(1));
                                break;

                            case Types.NULL:
                                LOG.info("read max is: {}", "");
                                break;
                            default:
                                LOG.info("read max is: {}", "");
                                break;
